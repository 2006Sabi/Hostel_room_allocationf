  ...
  const handleSubmit = async (e) => {
    ...
    if (user?.year === 1) {
      if (!twelfthPercentage) {
        return setError("12th Percentage is required for first-year students");
      }
      if (
        isNaN(twelfthPercentage) ||
        twelfthPercentage < 0 ||
        twelfthPercentage > 100
      ) {
        return setError("12th Percentage must be between 0 and 100");
      }
    }
    ...
    try {
      await api.post("/student/apply", {
        ...
        city: city,
        twelfthPercentage:
          user?.year === 1 ? Number(twelfthPercentage) : undefined,
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 mb-8 text-center">
          ...
          <p className="text-gray-500 mt-2">
            Submit your request for hostel accommodation.
          </p>
        </div>
        ...
        {!application && !success && (
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                  ...
                  >
                    ...
                    {rooms.map((room) => (
                      <option key={room._id} value={room._id}>
                        Block {room.block} (
                        {room.totalRooms - room.occupiedRooms} availability
                        left)
                      </option>
                    ))}
                  </select>
                ...
                {rooms.length === 0 && (
                  <p className="text-xs text-red-500 mt-2">
                    No rooms currently available to apply.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <p className="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-wider">
                    Distance will be automatically calculated for group
                    allocation.
                  </p>
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 uppercase tracking-wide mb-2">
                  Special Category (Priority Allocation)
                </label>
                <select
              ...
              {user?.year === 1 && (
                <div>
                  ...
                  />
                  <p className="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-wider">
                    Used for allocation priority ranking for first-year
                    students.
                  </p>
                </div>
              )}
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 uppercase tracking-wide mb-2">
                  Special Request{" "}
                  <span className="text-gray-400 text-xs font-normal lowercase">
                    (Optional)
                  </span>
                </label>
                <textarea
        ...
        {application && !success && (
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 text-center">
            ...
            <p className="text-gray-500 mb-6">
              You have an active application. You cannot submit multiple
